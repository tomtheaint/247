...
export const reviewerApi = {
  listAllGoals: (params?: { page?: number; limit?: number; q?: string; includeDeleted?: boolean }) =>
    client.get<PaginatedResponse<{ id: string; title: string; description?: string; icon: string; color: string; status: string; deletedAt: string | null; user: { id: string; username: string; displayName?: string }; milestones: { id: string; title: string; description?: string; dueDate?: string }[]; _count: { events: number; milestones: number } }>>("/reviewer/goals", { params }).then((r) => r.data),
